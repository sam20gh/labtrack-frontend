 ...
 *    (see CLAUDE.md, the fourth trap). "We could not reach Predyqt" is both true and
...
const SPEC: Record<StateKey, Omit<StateDescriptor, 'key' | 'badge' | 'detail'> & { badge?: string }> = {
    ...
    not_allowed: {
        ...
        badgeIcon: 'warning-outline',
        title: 'Not Allowed',
        body: 'This part of Predyqt is not available to your account. If that looks wrong, support can check it for you.',
        retryable: false,
    },
    locked: {
        tone: 'accent',
    ...
    update: {
        ...
        body: 'A newer version of Predyqt is ready. Update to carry on where you left off.',
        retryable: false,
    },
    empty: {
        tone: 'accent',
...
export const describeError = (error: unknown, options: DescribeOptions = {}): StateDescriptor => {
    ...
    if (status === 0) {
        return describeState('offline', {
            body: subject
                ? `We could not reach Predyqt to load ${subject}. Please check your connection and try again.`
                : SPEC.offline.body,
        });
    }
